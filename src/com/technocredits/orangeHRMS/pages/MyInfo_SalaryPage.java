...
public class MyInfo_SalaryPage extends PredefinedMethods {
	...
	public double getCTCValue() throws Exception {
		...
		double value = Double.parseDouble(getElementText("xpath", "//div[@id='salary_details_tab']//div[3]/div[2]", true).replace(",", ""));
		return value;
		//return Precision.round(value, 2);
	}
	
	public double getDeductionValue() {
		return Double.parseDouble(getElementText("xpath", "//div[@class='col s1 total'][contains(@ng-bind, 'deductions')]", true).replace(",", ""));
	}
	
	public double getTotalPayableValue() {
		return Double.parseDouble(getElementText("xpath", "//div[@class='col s1 total'][contains(@ng-bind, 'totalPayment')]", true).replace(",", ""));
	}
}
